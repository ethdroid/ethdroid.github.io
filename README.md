...
├── cape-shiller-2026-q2.png # CAPE/Shiller chart (Q2 2026 post)
├── chart-pce-prices.png    # PCE inflation chart (Q2 2026 post)
├── chart-employment-base.png # Employment base chart (Q2 2026 post)
├── chart-income-retail.png # Income & retail sales chart (Q2 2026 post)
├── letterhead-logo.png     # Lightened brand mark shown top-right of every post
└── README.md
```

Post images (like the CAPE chart) are dropped in the project root alongside HTML files and referenced by filename in `posts.json` — see section 2 for the schema.
...
### Market status pill (next to "Live Markets" header)

Automatically shows one of two states based on visitor's viewing time (converted to ET regardless of their timezone):

- **During US market hours** (Mon-Fri, 9:30 AM – 4:00 PM ET): green pulsing dot + "Live — US markets open"
- **Otherwise:** static dot + "As of [last trading day] at 4:00 PM ET"

Handles weekends and pre-open weekday hours correctly (e.g., Monday morning before 9:30 shows Friday's date, not Sunday's). Logic lives in `main.js` under the "Market status" comment block.
